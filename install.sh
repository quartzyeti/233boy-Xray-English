...
author=quartzyeti
# github=https://github.com/quartzyeti/xray
...
# root
[[ $EUID != 0 ]] && err "Current non-${yellow}ROOT user.${none}"
...
# systemd
[[ ! $(type -P systemctl) ]] && {
    err "This system lacks ${yellow}(systemctl)${none}, Please try to execute:${yellow} ${cmd} update -y;${cmd} install systemd -y ${none}to fix this error."
}
...
show_help() {
    ...
    echo -e "  -l, --local-install             Local installation, using the current directory"
    echo -e "  -p, --proxy <addr>              Use a proxy to download, e.g., -p http://127.0.0.1:2333 or -p socks5://127.0.0.1:2333"
    echo -e "  -v, --core-version <ver>        Customize $is_core_name version, e.g., -v v1.8.1"
...
install_pkg() {
    ...
    if [[ $cmd_not_found ]]; then
        ...
        msg warn "Installing Dependency Packages >${pkg}"
...
check_status() {
    ...
    [[ ! -f $is_pkg_ok ]] && {
        msg err "Failed to install dependency package"
        is_fail=1
    }
...
pass_args() {
    while [[ $# -gt 0 ]]; do
        ...
        -f | --core-file)
            [[ -z $2 ]] && {
                err "($1) Missing required parameters, correct usage example: [$1 /root/$is_core-linux-64.zip]"
            } || [[ ! -f $2 ]] && {
        ...
        -v | --core-version)
            [[ -z $2 ]] && {
                err "($1) Missing required parameters, correct usage examples: [$1 v1.8.1]"
            }
            is_core_ver=v${2#v}
            shift 2
            ;;
...
exit_and_del_tmpdir() {
    ...
    [[ ! $1 ]] && {
        ...
        echo -e "Feedback issues) https://github.com/${is_sh_repo}/issues"
        echo
        exit 1
    }
    exit
...
main() {
    ...
    [[ -f $is_sh_bin && -d $is_core_dir/bin && -d $is_sh_dir && -d $is_conf_dir ]] && {
        err "Script installation detected, if you need to reinstall it use the${green} ${is_core} reinstall ${none}command."
    }
    ...
    [[ $is_core_ver ]] && msg warn "${is_core_name} release: ${yellow}$is_core_ver${none}"
    [[ $proxy ]] && msg warn "Using Proxies: ${yellow}$proxy${none}"
    # create tmpdir
    mkdir -p $tmpdir
    # if is_core_file, copy file
    [[ $is_core_file ]] && {
        cp -f $is_core_file $is_core_ok
        msg warn "${yellow}${is_core_name} Documentation use > $is_core_file${none}"
    }
    # local dir install sh script
    [[ $local_install ]] && {
        >$is_sh_ok
        msg warn "${yellow}Local script installation > $PWD ${none}"
    }

    timedatectl set-ntp true &>/dev/null
    [[ $? != 0 ]] && {
        msg warn "${yellow}\e[4mReminder!!! It is not possible to set up automatic time synchronization, this may affect the use of the VMess protocol.${none}"
    }
    ...
    if [[ $is_core_file ]]; then
        ...
        [[ $? != 0 ]] && {
            msg err "${is_core_name} The file test failed."
            exit_and_del_tmpdir
        }
    ...
    # show a tips msg
    msg ok "Generating Configuration Files..."

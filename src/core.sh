...
create() {
    ...
    client)
        ...
        is_client=1
        get info $2
        [[ ! $is_client_id_json ]] && err "($is_config_name) Generating client configurations is not supported."
...
change() {
    ...
    [[ $is_dont_auto_exit ]] && {
        ...
        [[ $is_change_id ]] && {
            ...
            [[ $is_change_id == 'full' ]] && {
                [[ $3 ]] && is_change_msg="Changing Multiple Parameters" || is_change_msg=
            }
            [[ $is_change_msg ]] && _green "\nRapid Implementation: $is_change_msg"
        }
        info $1
    ...
    1)
        # new port
        is_new_port=$3
        [[ $host && ! $is_caddy ]] && err "($is_config_file) Changing ports is not supported, because it doesn't make sense.."
    ...
    5)
        # new uuid
        is_new_uuid=$3
        [[ ! $uuid ]] && err "($is_config_file) Changing the UUID is not supported."
        [[ $is_auto ]] && get_uuid && is_new_uuid=$tmp_uuid
        [[ ! $is_new_uuid ]] && ask string is_new_uuid "Please enter a new UUID:"
    ...
    9)
        # new remote port
        is_new_door_port=$3
        [[ $net != 'door' ]] && err "($is_config_file) Changing the destination port is not supported."
        [[ ! $is_new_door_port ]] && {
    ...
    10)
        ...
        if [[ $is_auto ]]; then
            ...
            [[ $is_new_private_key && ! $is_new_public_key ]] && {
                err "Public key could not be found."
            }
            [[ ! $is_new_private_key ]] && ask string is_new_private_key "Please enter the new Private key:"
            [[ ! $is_new_public_key ]] && ask string is_new_public_key "Please enter a new Public key:"
            if [[ $is_new_private_key == $is_new_public_key ]]; then
                err "Private key and Public key are not the same."
            fi
            ...
            if [[ $? != 0 ]]; then
                err "Private key doesn't pass the test."
            fi
            ...
            $is_core_bin -test <<<"$is_new_json" &>/dev/null
            if [[ $? != 0 ]]; then
                err "Public key doesn't pass the test."
            fi
            is_private_key=$is_new_private_key
    ...
    11)
        # new serverName
        is_new_servername=$3
        [[ ! $is_reality ]] && err "($is_config_file) Changing the serverName is not supported.."
        [[ $is_auto ]] && is_new_servername=$is_random_servername
        [[ ! $is_new_servername ]] && ask string is_new_servername "Please enter a new serverName:"
        is_servername=$is_new_servername
        [[ $(grep -i "^233boy.com$" <<<$is_servername) ]] && {
            err "What's wrong with you, man?"
        }
        add $net
        ;;
    12)
        ...
        if [[ $is_auto ]]; then
            ...
            [[ $is_new_dynamic_port_start && ! $is_new_dynamic_port_end ]] && {
                err "Dynamic termination port could not be found."
            }
    ...
    13)
        ...
        is_new_proxy_site=$3
        [[ ! $is_caddy && ! $host ]] && {
            err "($is_config_file) Changing the camouflage site is not supported."
        }
        [[ ! -f $is_caddy_conf/${host}.conf.add ]] && err "Unable to configure the camouflage site."
        [[ ! $is_new_proxy_site ]] && ask string is_new_proxy_site "Please enter the name of the new camouflage site (e.g. example.com):"
        ...
        msg "\nThe camouflage site has been updated to: $(_green $proxy_site) \n"
        ;;
    14)
        # new kcp seed
        is_new_kcp_seed=$3
        [[ ! $kcp_seed ]] && err "($is_config_file) Changing the mKCP seed is not supported."
        [[ $is_auto ]] && get_uuid && is_new_kcp_seed=$tmp_uuid
        [[ ! $is_new_kcp_seed ]] && ask string is_new_kcp_seed "Please enter a new mKCP seed:"
        kcp_seed=$is_new_kcp_seed
        add $net
        ;;
    15)
        # new socks user
        [[ ! $is_socks_user ]] && err "($is_config_file) Changing the user name (Username) is not supported."
        ask string is_socks_user "Please enter a new user name (Username):"
        add $net
        ;;
    esac
}
...
del() {
    ...
    if [[ ! $(ls $is_conf_dir | grep .json) && ! $is_change ]]; then
        warn "The current configuration directory is empty! Probably because you just deleted the last configuration file."
        is_conf_dir_empty=1
    fi
...
uninstall() {
    ...
    msg "Where does the script need to be improved? Please give us your feedback!"
    ...
    [[ $is_test_run && ! $is_new_install ]] && {
        ...
        if [[ ! $(pgrep -f $is_run_bin) ]]; then
            ...
            [[ ! $is_no_manage_msg ]] && {
                ...
                _yellow "Automatically executes test runs when a run failure is detected.."
                get test-run
                _yellow "The test is finished, please press Enter to exit."
            }
        fi
    }
}
...
api() {
    ...
    [[ $is_core_stop ]] && {
        warn "$is_core_name Currently in stop state."
        is_api_fail=1
        return
    }
    case $1 in
    ...
    [[ ! $is_api_port ]] && {
        ...
        [[ $? != 0 ]] && {
            warn "Failed to read API port, can't use API operation.."
            return
        }
    }
...
add() {
    ...
    *http)
    ...
    [[ $1 && ! $is_change ]] && {
        msg "\nUse of protocols: $is_new_protocol"
        # err msg tips
    ...
    if [[ $is_no_auto_tls && ! $is_use_tls ]]; then
        err "$is_new_protocol Manually configuring tls is not supported."
    fi
    ...
    if [[ $2 ]]; then
        ...
        if [[ $is_use_port ]]; then
            [[ ! $(is_test port ${is_use_port}) ]] && {
                err "($is_use_port) Not a valid port. $is_err_tips"
            }
            [[ $(is_test port_used $is_use_port) ]] && {
                err "Unable to use ($is_use_port) port. $is_err_tips"
            }
            port=$is_use_port
        fi
        if [[ $is_use_door_port ]]; then
            [[ ! $(is_test port ${is_use_door_port}) ]] && {
                err "(${is_use_door_port}) Not a valid target port. $is_err_tips"
            }
        ...
        if [[ $is_use_header_type || $is_use_method ]]; then
            ...
            [[ ! ${is_tmp_use_type} ]] && {
                warn "(${is_use_header_type}${is_use_method}) Not a usable${is_tmp_use_name}."
    ...
    if [[ $is_use_tls ]]; then
        if [[ ! $is_no_auto_tls && ! $is_caddy && ! $is_gen ]]; then
            ...
            [[ $(is_test port_used 80) || $(is_test port_used 443) ]] && {
                ...
                warn "port (80 or 443) is already occupied, you may also consider using no-auto-tls"
                msg "\e[41m no-auto-tls help (help)\e[0m: $(msg_ul https://233boy.com/$is_core/no-auto-tls/)\n"
                msg "\n Caddy will auto-configure TLS using non-standard ports, HTTP:$is_http_port HTTPS:$is_https_port\n"
                msg "请确定是否继续???"
                pause
            }
            is_install_caddy=1
        ...
        if [[ $is_main_start ]]; then
            ...
            *tcp* | *kcp* | *quic*)
            ...
            socks)
                # set user
                [[ ! $is_socks_user ]] && ask string is_socks_user "Please set the user name:"
                # set password
    ...
    if [[ $(grep 2022 <<<$ss_method) ]]; then
        ...
        [[ $ss_password ]] && {
            ...
            $is_core_bin -test <<<"$is_new_json" &>/dev/null
            if [[ $? != 0 ]]; then
                warn "Shadowsocks 协议 ($ss_method) The use of passwords is not supported ($(_red_bg $ss_password))\n\nYou can use the command: $(_green $is_core ss2022) Generate supported passwords.\n\nThe script will automatically create the available passwords:)"
                ss_password=
                # create new json.
                json_str=
            fi
...
get() {
    ...
    info)
        ...
        if [[ $is_config_file ]]; then
            ...
            [[ $? != 0 ]] && err "Unable to read this file: $is_config_file"
    ...
    protocol)
        ...
        *socks*)
            ...
            ;;
        *)
            err "Unable to recognize the protocol: $is_config_file"
            ;;
        esac
        ...
        *reality*)
            ...
            ;;
        *)
            err "Unable to recognize transport protocol: $is_config_file"
            ;;
        esac
        ;;
    ...
    dynamic-port-test) # test dynamic port
        [[ ! $(is_test port ${is_use_dynamic_port_start}) || ! $(is_test port ${is_use_dynamic_port_end}) ]] && {
            err "Unable to correctly handle dynamic port ($is_use_dynamic_port_start-$is_use_dynamic_port_end) range."
        }
        [[ $(is_test port_used $is_use_dynamic_port_start) ]] && {
            err "Dynamic port ($is_use_dynamic_port_start-$is_use_dynamic_port_end), though port ($is_use_dynamic_port_start) is not available."
        }
        [[ $(is_test port_used $is_use_dynamic_port_end) ]] && {
            err "Dynamic port ($is_use_dynamic_port_start-$is_use_dynamic_port_end), through port ($is_use_dynamic_port_end) is not available."
        }
    ...
    host-test) # test host dns record; for auto *tls required.
        ...
        if [[ ! $(grep $ip <<<$is_host_dns) ]]; then
            ...
            msg "\nIf using Cloudflare, in the DNS section; turn off DNS only Proxy status"
            ask string y "I have confirmed the resolution [y]:"
            get ping
            if [[ ! $(grep $ip <<<$is_host_dns) ]]; then
                _cyan "\Test results: $is_host_dns"
                err "Domain name ($host) is not resolving to ($ip)"
            fi
        fi
        ;;
    ssss | ss2022)
    ...
    ping)
        ...
        ;;
    log | logerr)
        msg "\n Reminder: Press $(_green Ctrl + C) to quit.\n"
        [[ $1 == 'log' ]] && tail -f $is_log_dir/access.log
        [[ $1 == 'logerr' ]] && tail -f $is_log_dir/error.log
        ;;
    install-caddy)
        _green "\nInstall Caddy to automate TLS configuration.\n"
        load download.sh
        download caddy
        load systemd.sh
    ...
    test-run)
        ...
        [[ $? != 0 ]] && {
            _yellow "\nCan't run the test, please check systemctl state.\n"
            return
        }
        is_no_manage_msg=1
        if [[ ! $(pgrep -f $is_core_bin) ]]; then
            ...
            if [[ $is_run_fail == $is_core ]]; then
                _red "$is_core_name Run Failure Message:"
                $is_core_bin run -c $is_config_json -confdir $is_conf_dir
            else
                _green "\nTest passed, activated. $is_core_name ..\n"
            fi
        else
            _green "\n$is_core_name Running, skipping tests\n"
        fi
        if [[ $is_caddy ]]; then
            if [[ ! $(pgrep -f $is_caddy_bin) ]]; then
                ...
                if [[ $is_run_fail == 'caddy' ]]; then
                    _red "Caddy Run Failure Message:"
                    $is_caddy_bin run --config $is_caddyfile
                else
                    _green "\nTest passed. Caddy is up and running. ..\n"
                fi
            else
                _green "\nCaddy's running. Skip the test.\n"
            fi
        fi
        ;;
    esac
...
info() {
    ...
    tcp | kcp | quic)
        ...
        [[ $is_dynamic_port ]] && {
            is_can_change+=(12)
            is_tmp_port="$port & dynamic Ports: $is_dynamic_port_range"
        }
        [[ $kcp_seed ]] && {
            is_info_show+=(9)
            is_can_change+=(14)
    ...
    if [[ $is_new_install ]]; then
        warn "For first time installation please check the script help file: $(msg_ul https://233boy.com/$is_core/$is_core-script/)"
    fi
    if [[ $is_url ]]; then
        msg "------------- ${info_list[12]} -------------"
...
footer_msg() {
    ...
    unset c n m s b
    msg "------------- END -------------"
    msg "Concerns (tg): $(msg_ul https://t.me/tg2333)"
...
url_qr() {
    ...
    if [[ $is_url ]]; then
        [[ $1 == 'url' ]] && {
            ...
            msg
            msg "If it does not display or recognize properly, please use the following link to generate a QR code:"
...
update() {
    ...
    2 | sh)
        ...
        ;;
    3 | caddy)
        [[ ! $is_caddy ]] && err "Updates to Caddy are not supported.."
    ...
    *)
    ...
    [[ $is_run_ver == $is_new_ver ]] && {
        msg "\nThe customized version is the same as the current $is_show_name version, no need to update it.\n"
        exit
    }
    load download.sh
    if [[ $is_new_ver ]]; then
        msg "\nUpdating $is_show_name with a customized version: $(_green $is_new_ver)\n"
    else
        get_latest_version $is_update_name
        [[ $is_run_ver == $latest_ver ]] && {
            msg "\n$is_show_name is currently up to date..\n"
            exit
        }
    ...
    msg "Successful update, current $is_show_name version: $(_green $is_new_ver)\n"
    msg "$(_green Please check the update notes: https://github.com/$is_update_repo/releases/tag/$is_new_ver)\n"
    [[ $is_update_name != 'sh' ]] && manage restart $is_update_name &
}

# main menu; if no prefer args.
is_main_menu() {
    msg "\n------- $is_core_name script $is_sh_ver by 233boy [$author's bad translation] -------"
    ...
    4)
        ...
        ;;
    5)
        ask list is_do_manage "Start Stop Reboot"
        manage $REPLY &
        msg "\nManagement status implementation: $(_green $is_do_manage)\n"
        ;;
    6)
        is_tmp_list=("Update $is_core_name" "Update Scripts")
...
main() {
    ...
    dd | ddel | fix | fix-all)
        ...
        *)
            is_dont_auto_exit=1
            [[ ! $2 ]] && {
                err "Unable to find the parameter to be deleted"
            } || {
                for v in ${@:2}; do
                    del $v
                done
    ...
    debug)
        is_debug=1
        get info $2
        warn "If you need to copy; please rewrite the values of *uuid, *password, *host, *key to avoid compromise."
        ;;
    fix-config.json)
        create config.json
        ;;
    ...
    u | up | update | U | update.sh)
        ...
        if [[ $2 == 'dat' ]]; then
            load download.sh
            download dat
            msg "$(_green Updating geoip.dat geosite.dat Succeeded.)\n"
            manage restart &
        else
    ...
    *)
        ...
        if [[ $is_change_id ]]; then
            ...
            err "Unrecognized ($1), for help use: $is_core help"
        fi
        ;;
    esac
}
